...
class OptionsFramework_Backup {
	...
	function register_admin_screen () {
		...
		$this->admin_page = add_theme_page(
			...
			'Theme Options Import/Export',
			'manage_options', $this->token, array( &$this, 'admin_screen' )
		);
...
?>
	<div class="wrap">
		...
		<h2><?php _e( 'Import / Export', 'OptionsFramework' ); ?></h2>
		
		<div class="import">
			<h3><?php _e( 'Import Settings', 'OptionsFramework' ); ?></h3>
			
			<p><?php _e( 'If you have settings in a backup file on your computer, the Import / Export system can import those into this site. To get started, upload your backup file to import from below.', 'OptionsFramework' ); ?></p>
				...
				<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
					...
					<label for="OptionsFramework-import-file"><?php printf( __( 'Upload File: (Maximum Size: %s)', 'OptionsFramework' ), ini_get( 'post_max_size' ) ); ?></label>
					...
					<input type="submit" class="button" value="<?php _e( 'Upload File and Import', 'OptionsFramework' ); ?>" />
				</form>

		</div>
		
		<div class="export">
			<h3><?php _e( 'Export Settings', 'OptionsFramework' ); ?></h3>
			
			<p><?php _e( 'When you click the button below, the Import / Export system will create a text file for you to save to your computer.', 'OptionsFramework' ); ?></p>
			<p><?php echo sprintf( __( 'This text file can be used to restore your settings here on "%s", or to easily setup another website with the same settings".', 'OptionsFramework' ), get_bloginfo( 'name' ) ); ?></p>
...
<?php
	...
	function admin_screen_help ( $contextual_help, $screen_id, $screen ) {
		...
		$contextual_help =
		  '<h3>' . __( 'Welcome to the OptionsFramework Backup Manager.', 'OptionsFramework' ) . '</h3>' .
		  '<p>' . __( 'Here are a few notes on using this screen.', 'OptionsFramework' ) . '</p>' .
		  '<p>' . __( 'The backup manager allows you to backup or restore your "Theme Options" and other settings to or from a text file.', 'OptionsFramework' ) . '</p>' .
		  '<p>' . __( 'To create a backup, simply select the setting type you\'d like to backup (or "All Settings") and hit the "Download Export File" button.', 'OptionsFramework' ) . '</p>' .
		  '<p>' . __( 'To restore your settings from a backup, browse your computer for the file (under the "Import Settings" heading) and hit the "Upload File and Import" button. This will restore only the settings that have changed since the backup.', 'OptionsFramework' ) . '</p>' .
		  
		  '<p><strong>' . __( 'Please note that only valid backup files generated through the OptionsFramework Backup Manager should be imported.', 'OptionsFramework' ) . '</strong></p>';
	...
	function admin_notices () {
		...
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			echo '<div id="message" class="error"><p>' . __( 'There was a problem importing your settings. Please Try again.', 'OptionsFramework' ) . '</p></div>';
		} else if ( isset( $_GET['error-export'] ) && $_GET['error-export'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.', 'OptionsFramework' ) . '</p></div>';
		} else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.', 'OptionsFramework' ) . '</p></div>';
	...
	function import() {
		...
		$upload = wp_handle_upload($_FILES['OptionsFramework-import-file'],$overrides);
		
		// Check if file is uploaded
		if(isset($upload['file'])) {		
			$getfile = wp_remote_get($upload['url']);		
		} else {
		...
		$datafile = json_decode( $getfile['body'], true );
		...
		$has_updated = true; // If this is set to true at any stage, we update the main options collection.
		...
		// Errors: update fail
			var_dump($optionsframework_name, $datafile);
			wp_redirect( admin_url( 'admin.php?page=' . $this->token . '&error=true' ) );
			exit;
		}
